...
async function processOneUpgrade(): Promise<boolean> {
    ...
    let pendingResult: { pendingCount: number; request: { id: string; userId: string; email: string; bookingId: string; fromFlightId: string; toFlightId: string | null; priceDifference: number; route: { from: string; to: string; airline: string }; fromCabin: string; toCabin: string | null; toFlightAvailable: boolean } | null } | null = null;
    ...
    if (!request.toFlightId || !request.toFlightAvailable) {
        console.log(`[upgrade-scheduler] No available Business class flight for ${request.fromFlightId} — skipping upgrade ${request.id}`);
        return false;
    }
...
async function runAgentServer() {
    ...
    const server = createServer(async (request, response) => {
        ...
        // Demo endpoint: trigger upgrade processing in the background.
        // Returns 202 immediately. Background task loops through all pending upgrades
        // one at a time — each waits for CIBA user approval before moving to the next.
        // Stops when no pending upgrades remain.
    ...
    server.listen(port, host, () => {
        ...
        // startUpgradeScheduler(); // use POST /api/demo/process-upgrades to trigger manually
    });
