...
const TOOL_SCOPES = {
  ...
  upgrade_booking: "booking:upgrade",
...
function createTravelMcpServer(user) {
  ...
  tool(
    "request_upgrade",
    "Queue a flight upgrade request for the current user. Only call this when find_upgrade_options shows canUpgradeDirectly: false. The upgrade scheduler will find the matching Business class flight and process it asynchronously.",
    {
      bookingId: z.string().describe("The ID of the existing Economy flight booking to upgrade.")
    },
    async ({ bookingId }) => {
  ...
  tool(
    ...
    async ({ upgradeRequestId }) => {
      ...
      // Resolve the business class flight at processing time (toFlightId is not stored at request time)
