...
function ensureSchema(database) {
  ...
  const flightCols = database.prepare("PRAGMA table_info(flights)").all();
  const hasAvailable = flightCols.some(c => c.name === 'available');
  if (!hasAvailable) {
    database.exec("ALTER TABLE flights ADD COLUMN available INTEGER NOT NULL DEFAULT 1");
    // Set Business class flights to unavailable (0)
...
export function findFlights({ from, to, cabin }) {
  ...
  // Only return directly bookable flights (available = 1).
  // Business class flights (available = 0) are accessible only through the upgrade flow.
...
export function createUpgradeRequest({ id, userId, email, bookingId, fromFlightId, createdAt }) {
  getDatabase()
    .prepare(
      `INSERT INTO upgrade_requests
         (id, user_id, username, booking_id, from_flight_id, status, created_at, updated_at)
       VALUES
         (@id, @userId, @email, @bookingId, @fromFlightId, 'pending', @createdAt, @createdAt)`
    )
    .run({ id, userId, email: email ?? null, bookingId, fromFlightId, createdAt });
...
export function getOnePendingUpgrade() {
  ...
  // Resolve the matching business class flight at processing time
...
export function getBookingById(bookingId) {
  const row = getDatabase()
    .prepare(`SELECT b.id AS booking_id, b.booking_reference, b.username, b.travelers, b.status, b.created_at, f.* FROM bookings b JOIN flights f ON b.item_id = f.id WHERE b.id = @bookingId AND b.type = 'flight'`)
    .get({ bookingId });
  if (!row) return null;
  return { id: row.booking_id, bookingReference: row.booking_reference, flight: mapFlight(row) };
}
